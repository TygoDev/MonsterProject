...
public class GameManager : MonoBehaviour
{
    ...
    private void ChangedActiveScene(Scene current, Scene next)
    {
        if (next.name.Contains("Level")) //change to level, we will call the scenes with puzzles Level_number
        {
            ...
            {
                var p1 = PlayerInput.Instantiate(playerPrefab, controlScheme: "Joystick", pairWithDevice: Gamepad.all[1]);
                var p2 = PlayerInput.Instantiate(playerPrefab, controlScheme: "Joystick", pairWithDevice: Gamepad.all[0]);
